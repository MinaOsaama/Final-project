    ...
    : QObject(parent), socketDescriptor(socketDescriptor), socket(nullptr), database(database),mutex(&mutex)
{
    ...
    {
        qDebug() << "Data Base loaded successfully.";
        m_clientsCount++;
        qDebug() << "There are "<<m_clientsCount<<" connected clients";
    }
}
...
void ClientHandler::start()
{
    ...
    qDebug() << "ClientHandler is running in thread:" << currentThread;
...
void ClientHandler::onDisconnected()
{
    ...
    // {
    //     qDebug() << "Socket not fully disconnected .";
    // }
    socket ->deleteLater();
}
